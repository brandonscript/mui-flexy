...
import { FlexBox } from "@mui-flexy/v5";
...
const _StyledStockBox = styled((props: WithDataTestId<BoxProps>) => (
  <MuiBox {...props} position="absolute" component="span" />
))<FlexBoxProps>(({ theme }) =>
  theme.unstable_sx({
    padding: 1,
    margin: 1,
...
const ForwardRefStockBox = React.forwardRef<HTMLDivElement, WithDataTestId<BoxProps>>((props, ref) =>
  withThemeProvider(() => <MuiBox ref={ref} {...props} position="absolute" />)(),
);
...
)) as React.ComponentType<WithDataTestId<FlexBoxColumnProps>>;
